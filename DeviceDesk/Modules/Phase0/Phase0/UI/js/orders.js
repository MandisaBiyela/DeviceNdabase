(function () {
  ...
  function syncDeliverySelectStyle(sel) {
    sel.className = "select-delivery " + deliveryStatusClass(sel.value);
  }
  ...
  function addItemRow(schoolCard) {
    ...
    const itemIdx = tbody.querySelectorAll(".order-item-row").length;
  ...
  function recalcSchoolSubtotal(schoolCard) {
    const rows = Array.from(schoolCard.querySelectorAll(".order-item-row"));
    let subtotal = 0;
    rows.forEach((row) => {
  ...
  function bindItemHandlers(schoolCard) {
    ...
    rows.forEach((row) => {
      ...
      const delivery = row.querySelector(".item-delivery-status");
  ...
  function validateForm() {
    ...
    const cards = Array.from(document.querySelectorAll(".school-shell"));
    if (cards.length === 0) {
      ok = false;
    ...
    cards.forEach((card) => {
      ...
      card.querySelectorAll(".order-item-row").forEach((row) => {
  ...
  function collectPayload() {
    const schools = Array.from(document.querySelectorAll(".school-shell")).map((card) => {
      const items = Array.from(card.querySelectorAll(".order-item-row")).map((row) => {
        const key = row.querySelector(".item-delivery-status").value;
        return {
